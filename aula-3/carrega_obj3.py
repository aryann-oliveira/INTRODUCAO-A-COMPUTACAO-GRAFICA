...
try:
    scene = pywavefront.Wavefront('tataruga.obj', collect_faces=True, create_materials=True)
...
def draw_scene(window):
    ...
    glRotatef(-90, 1, 0, 0)   # <-- deita a tartaruga
    glTranslatef(0.0, -1.0, 0.0)   
    


    # Rodando o dado pra bater a luz 
    glRotatef(glfw.get_time() * 30, 0, 0, 1)
...
def main():
    ...
    window = glfw.create_window(800, 600, "tataruga", None, None)
    if not window:
        glfw.terminate()
        return
